...
整理日期：2026-09-26。工作区产品版本为 2.9.0；最新测试数量如下，实际执行与发布状态以根目录项目日志为准。
...
2. `npm test`：47 项模型、筛选、草稿、版本判断、复访统计、选店对比与连接恢复测试。
3. `npm run check:release`：前端版本号、模块引用、离线缓存清单、唯一元素 ID、表单标签、保留用户缩放。
4. 在 Ubuntu 24.04 的 Chromium 中执行 66 项移动端浏览器回归，包括照片、离线、模态框、草稿、真实 Service Worker 更新/半响应弱网回退、复访小结、选店对比和登录异常恢复。
