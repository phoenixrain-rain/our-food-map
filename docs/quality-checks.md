...
整理日期：2026-09-23。工作区产品版本为 2.8.0；最新测试数量如下，发布状态以根目录项目日志为准。
...
2. `npm test`：40 项模型、筛选、草稿、版本判断、复访统计与选店对比测试。
3. `npm run check:release`：前端版本号、模块引用、离线缓存清单、唯一元素 ID、表单标签、保留用户缩放。
4. 在 Ubuntu 24.04 的 Chromium 中执行 56 项移动端浏览器回归，包括照片、离线、模态框、草稿、真实 Service Worker 更新、复访小结和选店对比。
