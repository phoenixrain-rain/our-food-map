...
try {
  ...
  await pageA.locator('[data-nav="profile"]').click(); await pageA.locator('[data-page="profile"] [data-open="connection"]').click();
  await expect(pageA.locator('#connectionTitle')).toContainText('已同步'); await expect(pageA.locator('#connectionLastSync')).not.toContainText('尚未');
  const connectionReport = await pageA.locator('#connectionReport').inputValue();
  for (const value of [space.id, space.invite_code, ...resources.users]) assert.ok(!connectionReport.includes(value), 'connection report must not include private identifiers');
  assert.ok(!connectionReport.includes('@example.invalid'), 'connection report must not include account email');
  await pageA.locator('#retryConnection').click(); await expect(pageA.locator('#retryConnection')).toBeEnabled({ timeout: 30000 });
  await expect(pageA.locator('#connectionTitle')).toContainText('已同步');
  await pageA.locator('[data-close="connectionSheet"]').click(); await pageA.locator('[data-nav="home"]').click();
  console.log('PASS connection recovery reads the same real test space and report excludes private identifiers');
  ...
  await pageA.locator('#detailBody [data-compare-add]').click(); await pageA.locator('#compareCandidates [aria-pressed="false"]').first().click(); await pageA.locator('#showComparison').click();
  await expect(pageA.locator('[data-compare-row="value"] td').first()).toContainText('8.5'); await expect(pageA.locator('[data-compare-row="price"] td').first()).toContainText('¥99');
  ...
  await expect(pageA.locator('[data-compare-row="value"] td').first()).toContainText('5.0', { timeout: 30000 }); await expect(pageA.locator('#compareSelected button')).toHaveCount(2);
  await expect(pageA.locator('#compareChooser')).not.toHaveAttribute('open', ''); await pageA.locator('[data-close="compareSheet"]').click();
  console.log('PASS open shortlist follows partner joint review in place, retains selections and independently dated spending');
  ...
  await pageA.locator('[data-nav="records"]').click(); await pageA.locator('[data-page="records"] [data-open="compare"]').click();
  await pageA.locator('#compareCandidates button').nth(0).click(); await pageA.locator('#compareCandidates button').nth(1).click(); await expect(pageA.locator('#compareResults table')).toHaveCount(1);
  await pageA.locator('#compareSearch').fill('退出前的私有搜索'); await pageA.locator('[data-close="compareSheet"]').click();
  ...
  for (const selector of ['#compareSelected', '#compareCandidates', '#compareResults', '#compareStatus', '#compareMatches']) await expect(pageA.locator(selector)).toBeEmpty();
  await expect(pageA.locator('#compareSearch')).toHaveValue(''); await expect(pageA.locator('#compareSource')).toHaveValue('all'); await expect(pageA.locator('#compareSheet')).not.toHaveClass(/open/);
  ...
  assert.equal(await pageA.evaluate(async scope => (await import('/lib/drafts.js?v=2.9.0')).loadDraft(scope), `cloud:${resources.users[0]}:${space.id}`), null, 'logout removes that account draft from IndexedDB');
