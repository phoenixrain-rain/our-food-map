...
export const emptyData = () => ({ nickname: '我', partnerName: 'TA', space: null, members: [], restaurants: [], reviews: [], photos: [] });
...
export function sortRestaurants(rows, data, key = 'recent') {
  return [...rows].sort((a, b) => {
    ...
    return (b.visit_date || b.created_at || '').localeCompare(a.visit_date || a.created_at || '') || String(a.id).localeCompare(String(b.id));
  });
}
...
export function validateBackup(raw) {
  ...
  if (d.restaurants.length > 10000 || d.reviews.length > 20000 || d.photos.length > 80000) throw new Error('备份记录数量超出范围');
  ...
  const restaurants = d.restaurants.map(r => {
    ...
    ids.add(r.id);
    return validateRestaurant({ id: text(r.id, 100), space_id: 'local-space', name: text(r.name, 120), city: text(r.city), category: text(r.category), address: text(r.address), visit_date: r.visit_date || null, price_per_person: numberOrNull(r.price_per_person), tags: Array.isArray(r.tags) ? r.tags.slice(0, 20).map(t => text(t, 40)) : [], status: r.status, created_at: text(r.created_at), updated_at: text(r.updated_at) });
  });
  ...
  return { ...emptyData(), nickname: text(d.nickname || '我', 40), partnerName: text(d.partnerName || 'TA', 40), restaurants, reviews, photos };
}
