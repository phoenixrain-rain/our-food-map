...
export const emptyData = () => ({ nickname: '我', partnerName: 'TA', space: null, members: [], restaurants: [], trash: [], reviews: [], photos: [] });
...
export function rankedRestaurants(data, key = 'value') {
  const records = [...new Map(data.restaurants.map(r => [r.id, r])).values()];
  return sortRestaurants(records.filter(r => !r.deleted_at && r.status === 'eaten' && summary(data, r).completeCount === 2 &&
    (key === 'price' ? numberOrNull(r.price_per_person) !== null : summary(data, r)[key] !== null)), data, key);
}
...
export function validateBackup(raw) {
  ...
  return { ...emptyData(), nickname: text(d.nickname || '我', 40), partnerName: text(d.partnerName || 'TA', 40), restaurants: restaurants.filter(r => !r.deleted_at), trash: restaurants.filter(r => r.deleted_at), reviews, photos };
}
