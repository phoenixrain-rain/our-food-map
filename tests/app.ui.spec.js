...
test('installed shell reloads offline and only removes its own old caches', async ({ page, context }) => {
  ...
  const keys = await page.evaluate(() => caches.keys()); expect(keys).toContain('other-application'); expect(keys).not.toContain('our-food-map-shell-v1'); expect(keys).toContain('our-food-map-shell-v10');
